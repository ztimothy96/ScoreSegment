...
def get_staves(im):
    ...
    im_bin_array = np.array(im_gray, dtype=np.float64) / 255.0
    # Apply threshold using numpy (matching cv2.THRESH_BINARY: > thresh -> 1.0, <= thresh -> 0.0)
    im_bin = (im_bin_array > thresh).astype(np.float64)
    ...
    staves = np.split(im_array, seps, axis=0)
    return staves
